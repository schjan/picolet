...
- Default credential model is one secret-provider token per Machine, placed into every Host's secrets directory by the bootstrap; the running Agent resolves its git credentials (token or GitHub App), and every other secret, through references in its Fleet-rendered config. Scoping provider tokens per Host (viewer role, only that Host's items), or giving a Host no provider and materializing its few secrets as files at bootstrap, is the documented stricter option for untrusted boundaries, not the default. The repairability rule is about hosting, not resolution: neither the Fleet repository nor the secret provider may be served by something the Agent itself deploys.
