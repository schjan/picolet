...
// uniqueTestQuadletDir returns a path under the standard rootless quadlet
// search root that no other concurrent e2e test will scan. Quadlet recurses
// through ~/.config/containers/systemd/, so units placed here are still
// generated by systemd; orphan scanners scoped to ~/.config/containers/systemd/picolet/
// (the standard picolet-owned subdir) cannot see this dir.
//
// The returned dir is registered for cleanup via t.Cleanup. We also re-run
// daemon-reload on cleanup so systemd drops any units generated from this
// dir before the next test starts — leaving stale generated units in
// systemd's runtime view would confuse orphan scans in later tests.
func uniqueTestQuadletDir(t *testing.T, systemd applier.SystemdManager) string {
	t.Helper()
	home, err := os.UserHomeDir()
	require.NoError(t, err)

	// Sanitize t.Name() to a filesystem-safe label. We keep only
	// [A-Za-z0-9_-]; everything else becomes '_'. os.Getpid() is what
	// actually guarantees uniqueness across test runs; the label is for
	// debuggability.
	...
	dir := filepath.Join(home, ".config", "containers", "systemd",
		fmt.Sprintf("picolet-test-%s-%d", safe, os.Getpid()))
	// 0o750 satisfies gosec G301 and matches the typical user-private mode
	// for files under ~/.config; quadlet only needs to read these as the
	// owning user.
	require.NoError(t, os.MkdirAll(dir, 0o750))

	t.Cleanup(func() {
		_ = os.RemoveAll(dir)
		// Best-effort daemon-reload to drop generated units from systemd's
		// view. Failures are not actionable inside cleanup.
		reloadCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
		_ = systemd.DaemonReload(reloadCtx)
	})
	return dir
}
...
`,
	...
	quadletDir := uniqueTestQuadletDir(t, systemd)
