...
// generated by systemd; orphan scanners scoped to
// ~/.config/containers/systemd/picolet/ (the standard picolet-owned subdir)
// cannot see this dir.
//
// The returned dir is RemoveAll'd in t.Cleanup. We do NOT issue a daemon-reload
// here: the per-test t.Cleanup that the caller registers AFTER this helper
// runs FIRST under LIFO order, and that cleanup already calls DaemonReload
// (followed by Close) — a second DaemonReload here would force the manager to
// reconnect, leak the new connection, and accomplish nothing.
func uniqueTestQuadletDir(t *testing.T) string {
	t.Helper()
	home, err := os.UserHomeDir()
	require.NoError(t, err)

	// os.Getpid() is what actually guarantees uniqueness across test runs;
	// the sanitized t.Name() is just a debuggable label.
...
`,
	...
	quadletDir := uniqueTestQuadletDir(t)
